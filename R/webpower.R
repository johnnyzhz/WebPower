  ...
  if (is.null(power)) 
    power <- eval(p.body) else if (is.null(J)) 
      J <- nuniroot(function(J) eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1 + 1e-10, 1000), interval))$root else if (is.null(n)) 
        n <- nuniroot(function(n) eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(3 - 1e-10, 1e+06), interval))$root else if (is.null(f) & type == "main") 
...
wp.logistic <- function(n = NULL, p0 = NULL, p1 = NULL, alpha = 0.05, power = NULL, 
  ...
  stop(sQuote(family), " unrecognized as a value for `family`")
  }
...
wp.poisson <- function(n = NULL, exp0 = NULL, exp1 = NULL, alpha = 0.05, 
  ...
  if (family == "Bernoulli") {
    ...
    stop(sQuote(family), " is not recognized value for `family`")
  }
