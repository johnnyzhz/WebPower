print.webpower <- function(x, ...) {
  ...
  note <- x$note
  url <- x$url
  x[c("method", "note", "url", "alternative", "family", "parameter")] <-
    NULL
  x <- do.call(cbind, x)
...
plot.webpower <-
  function(x,
           ...
           ...) {
    wp <- x
    wp[c("method", "note", "url", "alternative", "family", "parameter")] <-
      NULL
...
wp.anova <-
  function(k = NULL,
    ...
    if (!is.null(alpha) &&
        !is.numeric(alpha) || any(0 > alpha | alpha >
                                  1))
      stop(sQuote("alpha"), " must be numeric in [0, 1]")
    if (!is.null(power) &&
        !is.numeric(power) || any(0 > power | power >
                                  1))
    ...
    if (is.null(power))
      ...
      k <-
        uniroot(function(k)
          eval(p.body) - power, c(2 + 1e-10, 100))$root
    else if (is.null(n))
      n <-
        uniroot(function(n)
          eval(p.body) - power, c(2 + k + 1e-10,
                                  1e+05))$root
    else if (is.null(f))
      f <-
        uniroot(function(f)
...
wp.one.prop <-
  function(h = NULL,
    ...
    if (is.null(power))
      ...
      if (tside == 2) {
        h <- uniroot(function(h)
          eval(p.body) - power, c(1e-10, 10))$root
      }
      if (tside == 1) {
        h <- uniroot(function(h)
          eval(p.body) - power, c(-10, 5))$root
      }
      if (tside == 3) {
        h <- uniroot(function(h)
          eval(p.body) - power, c(-5, 10))$root
      }
    } else if (is.null(n))
      n <-
      uniroot(function(n)
        eval(p.body) - power, c(2 + 1e-10, 1e+05))$root
    else if (is.null(alpha))
      alpha <-
      uniroot(function(alpha)
        eval(p.body) - power, c(1e-10,
    ...
    if (tside == 2) {
      p.body <- quote({
        pnorm(qnorm(alpha / 2, lower = FALSE) - h * sqrt((n1 * n2) / (n1 +
                                                                        n2)), lower = FALSE) + pnorm(qnorm(alpha /
                                                                                                             2, lower = TRUE) -
                                                                                                       h * sqrt((n1 * n2) /
                                                                                                                  (n1 + n2)), lower = TRUE)
      })
...
wp.t1 <-
  function(n = NULL,
    ...
    NOTE <-
      switch(type, paired = "n is number of *pairs*", two.sample = "n is number in *each* group",
             NULL)
    METHOD <-
      paste(switch(
        type,
...
wp.rmanova <-
  function(n = NULL,
    ...
    if (!is.null(nscor) &&
        !is.numeric(nscor) || any(0 > nscor | nscor >
                                  1))
...
wp.effect.CRT2arm <- function(file) {
  ...
  mutj <-
    aggregate(dat.trt[, 3], by = list(dat.trt[, 2]), FUN = mean)
  mucj <-
    aggregate(dat.ctl[, 3], by = list(dat.ctl[, 2]), FUN = mean)
  ...
  muc <- mean(mucj[, 2])
  SSB <-
    (sum((mutj[, 2] - mut) ^ 2) + sum((mucj[, 2] - muc) ^ 2)) / (J - 2)
...
wp.effect.CRT3arm <- function(file) {
  ...
  mut1j <-
    aggregate(dat.trt1[, 3], by = list(dat.trt1[, 2]), FUN = mean)
  mut2j <-
    aggregate(dat.trt2[, 3], by = list(dat.trt2[, 2]), FUN = mean)
  mucj <-
    aggregate(dat.ctl[, 3], by = list(dat.ctl[, 2]), FUN = mean)
  ...
  muc <- mean(mucj[, 2])
  SSB <-
    (sum((mut1j[, 2] - mut1) ^ 2) + sum((mut2j[, 2] - mut2) ^ 2) + sum((mucj[,
                                                                             2] - muc) ^
                                                                         2)) / (J - 3)
  tau_hat <- SSB - SSW / n
...
wp.effect.MRT2arm <- function(file) {
  ...
  mutcj <-
    aggregate(dat[, 3], by = list(dat[, 2], dat[, 4]), FUN = mean)
  ...
  Xij <- dat[, 4] - 0.5
  sg2 <-
    sum((dat2[, 3] - dat2[, 5] - dat2[, 6] * Xij) ^ 2) / (J * (n - 2))
...
wp.effect.MRT3arm <- function(file) {
  ...
  dat2 <-
    merge(dat1, mut_c, by = names(dat)[2])  #merge beta1 and beta2
  ...
  sg2 <-
    sum((dat2[, 3] - dat2$beta0 - dat2$beta1 * X1ij - dat2$beta2 *
           X2ij) ^ 2) / (J * (n - 3))
  
  f1 <-
    (mud1 + mud2) / 2 / sqrt(sg2)  #effect size of treatment main effect
  f2 <-
    (mud1 - mud2) / sqrt(sg2)  #effect size of comparing the two treatments
  return(list(f1 = f1, f2 = f2))
}

nuniroot <- function(f, interval, maxlength = 100) {
  if (length(interval) != 2)
    stop("Please provide an interval with two values such as c(0,1).")
  x <- seq(min(interval), max(interval), length = maxlength)
  f.out <- f(x)
  if (min(f.out) * max(f.out) > 0)
    stop(
      "The specified parameters do not yield valid results. Please try to supply a different interval, e.g., using interval=c(0,1), for your parameter."
    )
  else {
...
wp.crt2arm <-
  function(n = NULL,
    ...
    if (is.null(power))
      ...
      J <-
        nuniroot(function(J)
          eval(p.body) - power, ifelse(is.null(interval), c(2 + 1e-10, 1000), interval))$root
    else if (is.null(n))
      n <-
        nuniroot(function(n)
          eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1, 1e+06), interval))$root
    else if (is.null(f))
      f <-
        nuniroot(function(f)
          eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-07, 1e+07), interval))$root
    else if (is.null(icc))
      icc <-
        nuniroot(function(icc)
          eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(0, 1), interval))$root
    else if (is.null(alpha))
      alpha <-
        nuniroot(function(alpha)
          eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-10, 1 - 1e-10), interval))$root
    else
      stop("internal error")
...
wp.crt3arm <-
  function(n = NULL,
    ...
    if (is.null(power))
      ...
      J <-
      nuniroot(function(J)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(3 + 1e-10, 1000), interval))$root
    else if (is.null(n))
      n <-
      nuniroot(function(n)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(2 + 1e-10, 1e+06), interval))$root
    else if (is.null(f))
      f <-
      nuniroot(function(f)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-07, 1e+07), interval))$root
...
wp.mrt2arm <-
  function(n = NULL,
    ...
    if (is.null(alpha) ||
        (!is.null(alpha) && !is.numeric(alpha) || any(0 >
                                                      alpha |
                                                      alpha > 1)))
      stop("alpha must be numeric in [0, 1]")
    if (!is.null(power) &&
    ...
    if (is.null(power))
      ...
      J <-
      nuniroot(function(J)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1 + 1e-10, 1000), interval))$root
    else if (is.null(n))
      n <-
      nuniroot(function(n)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(3 - 1e-10, 1e+06), interval))$root
    else if (is.null(f) & type == 1)
      f <-
      nuniroot(function(f)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-07, 1e+07), interval))$root
    else
...
wp.mrt3arm <-
  function(n = NULL,
    ...
    if (is.null(power))
      ...
      J <-
      nuniroot(function(J)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(2 - 1e-10, 1000), interval))$root
    else if (is.null(n))
      n <-
      nuniroot(function(n)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(3 - 1e-10, 1e+07), interval))$root
    else if (is.null(f1) && type != 2)
      f1 <-
      nuniroot(function(f1)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-07, 1e+07), interval))$root
    else if (is.null(f2) && type != 1)
      f2 <-
      nuniroot(function(f2)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-07, 1e+07), interval))$root
    else
      stop("internal error")
...
wp.regression <- function (n = NULL,
  ...
  if (is.null(power))
    ...
    n <- uniroot(function(n)
      eval(p.body) - power, c(5 +
                                p1 + 1e-10, 1e+05))$root
  else if (is.null(f2))
    f2 <- uniroot(function(f2)
      eval(p.body) - power, c(1e-07,
                              1e+07))$root
  else if (is.null(alpha))
    alpha <- uniroot(function(alpha)
      eval(p.body) - power,
...
wp.logistic <-
  function(n = NULL,
    ...
    if (is.null(p0) ||
        !is.null(p0) && !is.numeric(p0) || any(0 > p0 |
                                               p0 > 1))
      stop(sQuote("p0"), " must be numeric in (0,1)")
    
    if (is.null(p1) ||
        !is.null(p1) && !is.numeric(p1) || any(0 > p1 |
                                               p1 > 1))
    ...
    if (is.null(alpha) ||
        !is.null(alpha) & !is.numeric(alpha) || any(alpha <
                                                    0 |
                                                    alpha > 1))
      stop(sQuote("alpha"), " must be numeric in [0, 1]")
    if (!is.null(power) &
    ...
    if (family == "Bernoulli") {
      ...
      odds <- p1 / (1 - p1) / (p0 / (1 - p0))
      beta1 <-
        log(odds)  ##beta1 under H_A in the population##beta1 under H_A in the population
      d <-
        integrate(function(x)
          (1 - (1 + exp(-beta0 - beta1 * x)) ^ (-1)) *
            (1 + exp(-beta0 - beta1 * x)) ^ (-1) * dexp(x, lambda),
          0,
          Inf,
      ...
      e <-
        integrate(function(x)
          ...
          Inf,
          subdivisions = 100L)$value
      f <- integrate(function(x)
        x ^ 2 * (1 - (1 + exp(-beta0 - beta1 *
                                x)) ^ (-1)) * (1 + exp(-beta0 - beta1 * x)) ^
          (-1) * dexp(x, lambda),
        0,
        Inf,
        subdivisions = 100L)$value
      v1 <- d / (d * f - e ^ 2)
      mu1 <- integrate(function(x)
      ...
      if (is.null(parameter)) {
        ...
        if (length(parameter) != 2)
          stop(
            "Both mean and standard deviation of the log-normal distribution have to be provided as a vector."
          )
        mu <- parameter[1]
        sigma <- parameter[2]
      }
      ...
      a <- pn * (1 - pn)  #00
      b <-
        (exp(sigma * sigma) - 1) * exp(2 * mu + sigma ^ 2) * pn * (1 -
                                                                     pn)  #11
      ...
      if (is.null(parameter)) {
        ...
        if (length(parameter) != 2)
          stop(
            "Both mean and standard deviation of the normal distribution have to be provided as a vector."
          )
        mu <- parameter[1]
        sigma <- parameter[2]
      }
      ...
      d <-
        integrate(function(x)
          (1 - (1 + exp(-beta0 - beta1 * x)) ^ (-1)) *
            (1 + exp(-beta0 - beta1 * x)) ^ (-1) * dnorm(x, mu, sigma),
          -Inf,
          Inf,
          subdivisions = 100L)$value
      e <-
      ...
      d <-
        sum(sapply(0:1e+05, function(x)
          (1 - (
            1 + exp(-beta0 - beta1 *
                      x)
          ) ^ (-1)) * (1 + exp(-beta0 - beta1 * x)) ^ (-1) * dpois(x, lambda)))
      e <- sum(sapply(0:1e+05, function(x)
        x * (1 - (1 + exp(-beta0 -
                            beta1 * x)) ^
               (-1)) * (1 + exp(-beta0 - beta1 * x)) ^ (-1) * dpois(x,
                                                                    lambda)))
      f <-
        sum(sapply(0:1e+05, function(x)
          x ^ 2 * (1 - (1 + exp(-beta0 -
...
wp.mediation <-
  function(n = NULL,
    ...
    p.body <- quote({
      numer <- sqrt(n) * a * b
      denom <-
        sqrt(a ^ 2 * vary / (varm - a ^ 2 * varx) + b ^ 2 * (varm - a ^ 2 *
                                                               varx) / varx)
      delta <- numer / denom
      alpha2 <- alpha / 2
      za2 <- qnorm(1 - alpha2)
    ...
    if (is.null(power))
      ...
      a <-
        nuniroot(function(a)
          eval(p.body) - power, c(alow, aup))$root
    } else if (is.null(b))
      b <-
      nuniroot(function(b)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(-10, 10), interval))$root
    else if (is.null(varx))
      varx <-
      nuniroot(function(varx)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-10, 1e+07), interval))$root
    else if (is.null(vary))
      vary <-
      nuniroot(function(vary)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-10, 1e+07), interval))$root
    else if (is.null(varm))
      varm <-
      nuniroot(function(varm)
        eval(p.body) - power, ifelse(rep(is.null(interval), 2), c(1e-10, 1e+07), interval))$root
    else if (is.null(alpha))
      alpha <-
      nuniroot(function(alpha)
...
estCRT3arm <- function(file) {
  ...
  model <-
    summary(lmer(score ~ X1 + X2 + (1 | cluster), data = newdata))
  parest <- model$coefficients
  rownames(parest)[2] <- "X"
  ...
  # Omnibus test
  model1 <-
    lmer(score ~ X1 + X2 + (1 | cluster), data = newdata, REML = FALSE)
  model2 <-
    lmer(score ~ (1 | cluster), data = newdata, REML = FALSE)
...
estMRT3arm <- function(file) {
  ...
  newdata <- data.frame(dat, X1, X2)
  model <-
    summary(lmer(score ~ X1 + X2 + (X1 + X2 |
                                      cluster), data = newdata))
  parest <- model$coefficients
  rownames(parest)[2] <- "X"
  rownames(parest)[3] <- "X1-X2"
  J <- model$ngrps
  ...
  # Omnibus test
  model1 <-
    lmer(score ~ X1 + X2 + (X1 + X2 | cluster),
         data = newdata,
         REML = FALSE)
  model2 <-
    lmer(score ~ (X1 + X2 | cluster), data = newdata, REML = FALSE)
  omnibus <- anova(model1, model2)$"Pr(>Chisq)"[2]
  cat("Fixed effects (X: Treatment main effect; X1-X2: Comparing two treatments):\n")
  print(test)
  cat("Omnibust test: p-value = ")
...
wp.correlation <-
  function(n = NULL,
    ...
    if (tside == 3) {
      p.body <- quote({
        delta <- sqrt(n - 3 - p) * (log((1 + r) / (1 - r)) / 2 + r / (n -
                                                                        1 - p) /
                                      2 * (1 + (5 + r ^ 2) / (n - 1 - p) / 4 + (11 + 2 * r ^ 2 +
                                                                                  3 * r ^
                                                                                  4) / (n - 1 - p) ^ 2 / 8) - log((1 + rho0) / (1 - rho0)) / 2 -
                                      rho0 / (n - 1 - p) / 2)
        v <- (n - 3 - p) / (n - 1 - p) * (1 + (4 - r ^ 2) / (n - 1 - p) /
                                            2 +
                                            (22 - 6 * r ^ 2 - 3 * r ^ 4) / (n - 1 - p) ^
                                            2 / 6)
    ...
    if (is.null(power))
      ...
      n <-
      uniroot(function(n)
        eval(p.body) - power, c(4 + p + 1e-10,
                                1e+07))$root
    else if (is.null(r)) {
      if (tside == 2) {
        r <- uniroot(function(r)
          eval(p.body) - power, c(1e-10, 1 -
                                    1e-10))$root
      } else {
        r <- uniroot(function(r)
          eval(p.body) - power, c(-1 + 1e-10,
...
wp.poisson <-
  function(n = NULL,
    ...
    if (is.null(exp0) ||
        !is.null(exp0) && !is.numeric(exp0) || exp0 <=
        0)
      stop(sQuote("exp0"), " must be numeric in (0,Infinity)")
    
    if (is.null(exp1) ||
        !is.null(exp1) && !is.numeric(exp1) || exp1 <=
        0)
    ...
    if (family == "Bernoulli") {
      ...
      beta1 <- log(exp1)  ##beta1 under H_A in the population
      beta0 <- log(exp0)
      d <- integrate(function(x)
        exp(beta0 + (beta1 - lambda) * x) *
          lambda, 0, Inf, subdivisions = 100L)$value  ##00
      e <-
        integrate(function(x)
          x * exp(beta0 + (beta1 - lambda) * x) *
            lambda, 0, Inf, subdivisions = 100L)$value  #01
      f <- integrate(function(x)
        x ^ 2 * exp(beta0 + (beta1 - lambda) *
      ...
      f <-
        integrate(function(x)
          x ^ 2 * exp(beta0 + beta1 * x) * dlnorm(x,
                                                  mu, sigma),
          0,
          Inf,
          subdivisions = 100L)$value
      ...
      d <-
        sum(sapply(0:1e+05, function(x)
          exp(beta0 + beta1 * x) * dpois(x,
                                         lambda)))
      e <-
        sum(sapply(0:1e+05, function(x)
          x * exp(beta0 + beta1 * x) *
            dpois(x, lambda)))
      f <- sum(sapply(0:1e+05, function(x)
        x ^ 2 * exp(beta0 + beta1 *
                      x) * dpois(x, lambda)))
      v1 <- d / (d * f - e ^ 2)
      ...
      d <-
        integrate(function(x)
          exp(beta0 + beta1 * x) / (R - L), L, R,
          subdivisions = 100L)$value
      e <- integrate(function(x)
        x * exp(beta0 + beta1 * x) / (R - L),
        L, R, subdivisions = 100L)$value
      f <-
        integrate(function(x)
          x ^ 2 * exp(beta0 + beta1 * x) / (R - L),
          L,
          R,
          subdivisions = 100L)$value
      v1 <- d / (d * f - e ^ 2)
...
wp.sem.chisq <-
            ...
            alpha = 0.05)
  {
    ...
    if (is.null(power))
      ...
      n <- uniroot(function(n)
        eval(p.body) - power, c(10 +
                                  1e-10, 1e+08))$root
    else if (is.null(df))
      df <- uniroot(function(df)
        eval(p.body) - power, c(1,
                                10000))$root
    else if (is.null(alpha))
      alpha <- uniroot(function(alpha)
        eval(p.body) -
      ...
      effect <-
      uniroot(function(effect)
        eval(p.body) - power, c(0, 1))$root
    else
      stop("internal error")
...
wp.sem.rmsea <-
  function (n = NULL,
    ...
    if (sum(sapply(list(n, df, power, alpha, rmsea0, rmsea1), is.null)) != 1)
      stop(
        "exactly one of sample size, degrees of freedom, rmsea0, rmsea1, alpha and power must be NULL"
      )
    if (!is.null(alpha) && !is.numeric(alpha) || any(0 >
                                                     alpha |
                                                     alpha > 1))
    ...
    if (is.null(power))
      ...
      rmsea0 <- uniroot(function(rmsea0)
        eval(p.body) -
          power, c(0, 1))$root
    else if (is.null(rmsea1))
      rmsea1 <- uniroot(function(rmsea1)
        eval(p.body) -
          power, c(0, 1))$root
    else
      stop("internal error")
...
wp.anova.binary <-
  function(k = NULL,
    ...
    if (is.null(power))
      ...
      n <-
      uniroot(function(n)
        eval(p.body) - power, c(2 + k + 1e-10, 1e+05))$root
    else if (is.null(V))
      V <-
      uniroot(function(V)
        eval(p.body) - power, c(1e-07, 1e+07))$root
      ...
      alpha <-
      uniroot(function(alpha)
        eval(p.body) - power, c(1e-10, 1 - 1e-10))$root
    else
      stop("internal error")
...
wp.lcsm <- function(N = 100,
  ...
  fit.model <- function(T) {
    ...
    ## variance and covariance for intercept and slope
    model <-
      paste(model, "ys~~start(", vary0ys, ")*y0 + vary0ys*y0\n", sep = "")
    model <-
      paste(model, "y0~~start(", vary0, ")*y0 + vary0*y0\n", sep = "")
    model <-
      paste(model, "ys~~start(", varys, ")*ys + varys*ys\n", sep = "")
    
    model <-
      paste(model, "ys~start(", mys, ")*1 + label('mys')*1\n", sep = "")
    model <-
      paste(model, "y0~start(", my0, ")*1 + label('my0')*1\n", sep = "")
  ...
  fit.once <- function(N, T) {
    ...
    mc.se <-
      apply(all.res[, (npar + 1):(2 * npar)], 2, mean, na.rm = TRUE)
    ...
    pop.par <-
      unlist(lapply(label, function(x) {
        eval(parse(text = x))
      }))
    mc.output <-
      cbind(pop.par, mc.est, mc.sd, mc.se, mc.power, N, T)
...
plot.lcs.power <- function(x, parameter, ...) {
  ...
  if (length(unique.N) == 1 &
      length(unique.T) == 1)
    stop("Multiple N or T is needed for power plot.")
  ...
  if (length(unique.N) == 1) {
    ...
    plot(
      ...
      ylab = paste('Power of ', parameter, sep = ""),
      ylim = c(0, 1)
    )
...
wp.blcsm <- function(N = 100,
  ...
  fit.model <- function(T) {
    ...
    ## variance and covariance for intercept and slope
    model <-
      paste(model, "xs~~start(", varx0xs, ")*x0 + varx0xs*x0\n", sep = "")
    model <-
      paste(model, "x0~~start(", varx0, ")*x0 + varx0*x0\n", sep = "")
    model <-
      paste(model, "xs~~start(", varxs, ")*xs + varxs*xs\n", sep = "")
    
    model <-
      paste(model, "xs~start(", mxs, ")*1 + label('mxs')*1\n", sep = "")
    model <-
      paste(model, "x0~start(", mx0, ")*1 + label('mx0')*1\n", sep = "")
    ...
    model <-
      paste(model, "x0~~start(", varx0y0, ")*y0 + varx0y0*y0\n", sep = "")
    model <-
      paste(model, "x0~~start(", varx0ys, ")*ys + varx0ys*ys\n", sep = "")
    model <-
      paste(model, "y0~~start(", vary0xs, ")*xs + vary0xs*xs\n", sep = "")
    model <-
      paste(model, "xs~~start(", varxsys, ")*ys + varxsys*ys\n", sep = "")
...
summary.power <- function(object, ...) {
  ...
  if (object$out@Options$test %in% c("satorra.bentler",
                                     "yuan.bentler",
                                     "mean.var.adjusted",
                                     "scaled.shifted") &&
  ...
  print.estimate <-
    function(name = "ERROR",
      ...
      txt <-
        sprintf(
          "    %-15s %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f\n",
          name,
          pop.value[i],
          est[i],
          mse[i],
  ...
  for (g in 1:object@Data@ngroups) {
    ...
    # 2. regressions
    eqs.idx <-
      which(object@ParTable$op == "~" & object@ParTable$group == g)
    if (length(eqs.idx) > 0) {
      cat("Regressions:\n")
      lhs.old <- ""
    ...
    if (length(th.idx) > 0) {
      ...
      NAMES[th.idx] <-
        makeNames(
          paste(object@ParTable$lhs[th.idx],
                "|",
    ...
    if (length(delta.idx) > 0) {
      cat("Scales y*:\n")
      NAMES[delta.idx] <-
        makeNames(object@ParTable$rhs[delta.idx],
...
wp.mc.sem.basic <-
  function(model,
    ...
    if (length(skewness) > 1) {
      if (length(skewness) != length(ovnames))
        stop("The number of skewness is not equal to the number observed variables")
      index <- match(ov, ovnames)
      skewness <- skewness[index]
    }
    
    if (length(kurtosis) > 1) {
      if (length(kurtosis) != length(ovnames))
        stop("The number of kurtosis is not equal to the number observed variables")
      index <- match(ov, ovnames)
    ...
    cnames <-
      paste(temp.res@ParTable$lhs[idx],
            temp.res@ParTable$op[idx],
            temp.res@ParTable$rhs[idx])
    out <- temp.res
    runonce <- function(i) {
    ...
    res <- if (ncore > 1L && (have_mc || have_snow)) {
      if (have_mc) {
        ...
        if (is.null(cl)) {
          ...
          parallel::parLapply(cl, seq_len(RR), runonce)
      }
    } else
      lapply(seq_len(RR), runonce)
  ...
  function(model,
    ...
    ci.bc1 <- function(x, b, cl = .95) {
      ...
      names(qs) <- paste(
        if (np < 100)
          ...
          format(100 * alpha1, trim = TRUE, digits = dig),
        "%",
        sep = ""
      )
      qs
    ...
    runonce <- function(i) {
      ...
      if (class(temp.res) != "try-error") {
        ...
        temp.cvg <-
          (ci.res[, 4] >= par.value[idx]) & (ci.res[, 3] < par.value[idx])
...
wp.mmrm.ar1 <-
                            ...
                            "one.sided"))
  {
    ...
    n.body <- quote({
      ...
      pnorm(delta / sigma * sqrt(Na / ((
        phia + lambda * phib
      ))) +  qnorm(ifelse(
        alternative == "two.sided", alpha / 2, alpha
      )))
    })
    ...
    if (is.null(alpha))
      ...
      rho <- uniroot(function(rho)
        eval(n.body) - power, c(1e-10,
                                1 - 1e-10))$root
    else
      power <- eval(n.body)
    
    NOTE <-
      "Based power.mmrm.ar1 in longpower (Lu, Luo, & Chen ,2008)"
...
wp.mc.t <-
  function(n = NULL,
           ...
           alternative = c("two.sided",
                           "less", "greater")) {
    if (sum(sapply(
      list(n, mu0, R0, R1, mean, sd, skewness, kurtosis, alpha),
      is.null
    )) >= 1)
    ...
    if (tsample == 1) {
      ...
      data0 <- matrix(rpearson(n * R0, moments = moments0),
                      ncol = n,
                      nrow = R0)
      ...
      dataa <- matrix(rpearson(n * R1, moments = momentsa),
                      ncol = n,
                      nrow = R1)
      ...
      data01 <-
        matrix(rpearson(n[1] * R0, moments = moments01),
               ncol = n[1],
               nrow = R0)
      data02 <-
        matrix(rpearson(n[2] * R0, moments = moments02),
               ncol = n[2],
               nrow = R0)
      ...
      dataa1 <-
        matrix(rpearson(n[1] * R1, moments = momentsa1),
               ncol = n[1],
               nrow = R1)
      dataa2 <-
        matrix(rpearson(n[2] * R1, moments = momentsa2),
               ncol = n[2],
               nrow = R1)
    ...
    if (tsample == 2) {
      structure(
        list(
          ...
          sd1 = sd[1],
          sd2 = sd[2],
          skewness1 = skewness[1],
          skewness2 = skewness[2],
          kurtosis1 = kurtosis[1],
          kurtosis2 = kurtosis[1],
          alpha = alpha,
          alternative = alternative,
          note = NOTE,
          method = METHOD,
